...
class GBEXindex(TemplateView):
	...
	def data_counts(self):
		all_numbers = {model.__name__: [model.objects.filter(archived=False).count(), 0] for model in apps.get_app_config('GBEX_app').get_models() if getattr(model, "model_kind", False) == 'GBEX_Page'}
